...
TITLES_ZH=("" "反愿景设计师" "隐藏目标解码器" "身份建构追踪器" "生活方式—结果审计师" "认知失调引擎" "目标反馈调试器" "自我阶段导航员" "人生游戏架构师" "条件反射挖掘者" "一日重启设计师")
...
phase_count() { grep -cE "^## (Phase|Фаза|阶段) [0-9]+" "$1" 2>/dev/null || echo 0; }

# Extract phase content and clean up markdown headers for cleaner display
phase_content() {
    awk '/^## (Phase|Фаза|阶段) '"$2"'/{f=1;next}/^## (Phase|Фаза|阶段|Summary|总结)/{if(f)exit}f' "$1" | \
    sed -E 's/^### (Introduction|Вступление|引言)/\n/; s/^### (Questions|Вопросы|问题)/\n**问题：**\n/; s/^### (Question|Вопрос|单个问题)$/\n/; s/^### (Transition|Переход|过渡)/\n---\n/; s/^### (Task|Задание|任务)/\n**任务：**\n/; s/^### (Tasks|Задания|任务列表)/\n**任务：**\n/; s/^### (Analysis|Анализ|分析)/\n**分析：**\n/; s/^### (Information and Tasks|Информация и Задания|信息与任务)/\n/; s/^### .*/\n/'
}
...
write_insight() {
    ...
    if [[ -f "$session_file" ]]; then
        ...
        while IFS= read -r line; do
            if [[ "$line" =~ ^##\ (Phase|Фаза|阶段)\  ]]; then
                phase_num=$(echo "$line" | sed -E 's/^## (Phase|Фаза|阶段) ([0-9]+).*/\2/')
            elif [[ -n "$line" && ! "$line" =~ ^# && ! "$line" =~ ^--- && ! "$line" =~ ^(Started|Начало|开始时间) ]]; then
                local snippet="${line:0:200}"
                [[ ${#line} -gt 200 ]] && snippet="${snippet}..."
                if [[ "$lang" == "zh" ]]; then
                    echo "- **阶段 $phase_num：** $snippet" >> "$insights_file"
                elif [[ "$lang" == "ru" ]]; then
...
save_resp() {
    ...
    local lang=$(get lang)
    if [[ "$lang" == "zh" ]]; then
        [[ ! -f "$f" ]] && echo -e "# 第 $1 次：$(title "$1")\n开始时间：$ts\n" > "$f"
        echo -e "## 阶段 $2\n\n$3\n\n---\n" >> "$f"
    elif [[ "$lang" == "ru" ]]; then
        [[ ! -f "$f" ]] && echo -e "# Сессия $1: $(title "$1")\nНачало: $ts\n" > "$f"
        echo -e "## Фаза $2\n\n$3\n\n---\n" >> "$f"
    else
...
create_reminders() {
    ...
    if [[ "$lang" == "zh" ]]; then
        local questions=(
            "我现在做这些事情，是在逃避什么？"
            "如果有人拍下我过去两小时的表现，他会认为我想从人生中得到什么？"
            "我正在走向自己讨厌的人生，还是想要的人生？"
            "我假装不重要、但其实最重要的事情是什么？"
            "今天哪些行为是为了保护某种身份，而不是出于真实愿望？"
            "今天什么时候我最有活力？什么时候最麻木、最没有生命力？"
            "如果我不再需要别人以某种方式看待我，会发生什么变化？"
            "我在哪些地方用活力交换了安全感？"
        )
    elif [[ "$lang" == "ru" ]]; then
        local questions=(
...
case "$COMMAND" in
    ...
    reset)
        ...
        init_state "zh" && echo '{"status":"reset"}' ;;
    callback)
        case "${1:-}" in
            life:prev)
                p=$(get currentPhase)
            ...
            life:lang:zh) upd '.lang="zh"' && output ;;
            life:begin) release_lock; exec "$0" start zh "$WORKSPACE" ;;
            life:begin:en) release_lock; exec "$0" start en "$WORKSPACE" ;;
            life:begin:ru) release_lock; exec "$0" start ru "$WORKSPACE" ;;
            life:begin:zh) release_lock; exec "$0" start zh "$WORKSPACE" ;;
            life:session:*) release_lock; exec "$0" session "${1##life:session:}" "$WORKSPACE" ;;
            *) echo '{"status":"error","message":"Unknown callback"}'; exit 1 ;;
        esac ;;
    lang)
        [[ "${1:-zh}" =~ ^(en|ru|zh)$ ]] || { echo '{"status":"error","message":"支持的语言：zh、en、ru"}'; exit 1; }
        upd ".lang=\"$1\"" && output ;;
    reminders)
